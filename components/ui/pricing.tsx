...
function Star({
  ...
  const [initialPos, setInitialPos] = useState<{ top: string; left: string } | null>(null);
  const [starSize, setStarSize] = useState<{ width: string; height: string } | null>(null);
  const [animationParams, setAnimationParams] = useState<{ duration: number; delay: number } | null>(null);
  ...
  if (!initialPos || !starSize || !animationParams) return null;
  ...
  return (
    <PricingContext.Provider value={{ isMonthly, setIsMonthly }}>
      <div
        ...
        className={cn("relative w-full bg-background dark:bg-neutral-950 py-20 sm:py-24", className)}
      >
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col text-center">
        ...
        <ul
          role="list"
          className="mt-8 space-y-3 text-sm leading-6 text-left text-gray-300"
        >
        ...
        <div className="mt-auto pt-8">
          <Link
            ...
            className={cn(
              ...
              "w-full",
              !plan.isPopular && "text-white hover:text-black"
            )}
          >
            {plan.buttonText}
          </Link>
